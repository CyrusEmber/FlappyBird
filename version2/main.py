...
from load import *
from version2.saveLoad import save
from version2.settings import FIRST_POPULATION
...
cloud = new_clouds(batch=main_batch)
...
def init_pop():
    ...
    pop = create_pop(FIRST_POPULATION)
    create_ai_bird(pop)
...
def update(dt):
    ...
    for b in ai_birds_obj:
        if len(pillars) > 0:
            if b.collide_down(pillars[0]) or b.collide_up(pillars[1]):
                # add a little weight
                # b.score -= abs(b.x - pillars[0].x - SPACE / 2) * 0.001
                b.dead = True
        b.update(dt)
        # flap or not
    ...
    # evolve AI
    if alive == 0:
        pop = evolve(pop, 0.03, 10, 20)
        clear_game()
        create_ai_bird(pop)

    # kill bird for test
    if alive == 1 and time_count > 240 * 30:
        for obj in ai_birds_obj:
            alive -= 1
            obj.dead = True
